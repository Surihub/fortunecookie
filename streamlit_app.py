...
if open_cookie:
    ...
    # OpenAI API 프롬프트
    prompt = f"오늘 모의고사를 보는 학생들에게 힘을 줄 수 있는 감동적인 응원을 해줘. 편안한 마음으로 보되 그래도 긴장을 너무 안하진 말고 끝까지 열심히 풀고 최선을 다 할 수 있도록! 학생들의 기분은 '{mood}'이므로 이 기분을 고려해서 그에 적절한 응원을 해줘야해. 세 문장으로 말해주고 첫 문장은 공감으로 시작해. 학생의 무드를 한번 더 얘기해주는거야. 예를 들어 지금 긴장되는구나! 이런 식으로.. 그리고 말투는 학생들에게 하는 말이니까 극존칭은 아니어도 존대를 해줘. 그리고 모의고사 볼 때 도움이 되는 말을 해줘. 진심이 담긴 응원을!"
    ...
    st.balloons()
    st.warning(f"오늘의 운세: **{fortune}** *generated by GPT & 숩쌤*", icon="🔮")